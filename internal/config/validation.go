...
func DiscardNativeOnly(c ClusterResources) error {
	// peerEntry holds a BGPPeer together with its pre-compiled nodeSelectors so
	// that compileNodeSelectors is called at most once per peer.
	type peerEntry struct {
		peer     metallbv1beta2.BGPPeer
		compiled compiledNodeSelectors
	}
	...
	peerAddr := make(map[string][]peerEntry)
	...
	for i, p := range c.Peers {
		...
		compiledP, err := compileNodeSelectors(p.Spec.NodeSelectors)
		if err != nil {
			return fmt.Errorf("peer %s has invalid nodeSelector: %w", peerID, err)
		}
		for _, existing := range peerAddr[peerID] {
			if peersShareNodeCompiled(existing.compiled, compiledP, c.Nodes) {
				return fmt.Errorf("peer %s already exists, FRR mode doesn't support duplicate BGPPeers", peerID)
			}
		}
		peerAddr[peerID] = append(peerAddr[peerID], peerEntry{peer: p, compiled: compiledP})
	}
	return nil
}
...
// peersShareNodeCompiled returns true if any node matches both pre-compiled
// nodeSelector sets, meaning the two peers would be configured on the same FRR
// instance. Conservatively returns true when no nodes are known.
func peersShareNodeCompiled(c1, c2 compiledNodeSelectors, nodes []corev1.Node) bool {
	if len(nodes) == 0 {
		return true
	}
	for _, node := range nodes {
		nodeLabels := labels.Set(node.Labels)
		if c1.matches(nodeLabels) && c2.matches(nodeLabels) {
			return true
		}
	}
	return false
